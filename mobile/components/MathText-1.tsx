...
export function MathText({
  ...
  return (
    ...
    >
      {nodes.map((node, idx) => renderNode(node, idx, color))}
    </Text>
  );
}
...
function renderNode(node: InlineNode, idx: number, color: string) {
  if (node.type === 'break') return node.paragraph ? '\n\n' : '\n';
  if (node.type === 'text') {
    return (
      <Text key={idx} style={{ color, fontFamily: node.bold ? fonts.bold : fonts.regular }}>
        {node.value}
      </Text>
    );
  }
...
function toSupUnicode(s: string): string | null {
  let out = '';
  for (const ch of s) {
...
function toSubUnicode(s: string): string | null {
  let out = '';
  for (const ch of s) {
